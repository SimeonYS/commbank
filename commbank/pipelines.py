...
class CcommbankPipeline:

    # Database setup
    conn = sqlite3.connect('commbank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `commbank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM commbank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO commbank (date, title, link, content)"
